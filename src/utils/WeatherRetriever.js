...
export async function getWeatherDataByCoords(coords) {
    return await fetch(`${weatherApi.baseUrl}weather?lat=${coords.latitude}&lon=${coords.longitude}&units=imperial&appid=${weatherApi.key}`)
        .then(response => response.json())
        .then(result => {
            return result;
        });
